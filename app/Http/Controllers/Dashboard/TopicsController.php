...
class TopicsController extends Controller
{
    ...
    public function index($webmasterId)
    {
        ...
        if (!empty($WebmasterSection)) {
            return view("dashboard.topics.list", compact("GeneralWebmasterSections", "WebmasterSection"));
        } else {
    ...
    public function list(Request $request)
    {
        ...
        if ($find_date != "") {
            $Topics = $Topics->where("date", date('Y-m-d', strtotime($find_date)));
        }
        ...
        if ($totalFiltered > 0) {
            ...
            foreach ($Topics as $Topic) {
                ...
                if ($WebmasterSection->title_status) {
                    $nestedData['id'] = $photo . "<div class='h6'>" . $icon . $title . "</div>" . $section;
                }
                        ...
                        if ($customField->in_table) {
                            ...
                            if (($cf_saved_val != "" || count($cf_saved_val_array) > 0) && ($customField->lang_code == "all" || $customField->lang_code == @Helper::currentLanguage()->code)) {
                                if ($customField->type == 12) {
                                    ...
                                    $cf_data = date('Y-m-d H:i:s', strtotime($cf_saved_val));
                                } elseif ($customField->type == 4) {
                                    $cf_data = date('Y-m-d', strtotime($cf_saved_val));
                                } else {
                ...
                if (@$Topic->webmasterSection->date_status) {
                    $nestedData['date'] = "<div class=\"text-center\">" . $Topic->date . "</div>";
                }
        ...
        foreach ($WebmasterSection->customFields as $customField) {
            if ($customField->in_statics && ($customField->type == 6 || $customField->type == 7)) {
                ...
                foreach ($cf_details_lines as $cf_details_line) {
                    ...
                    if ($q != "") {
                        $stids = TopicField::select("topic_id")->where("field_value", 'like', '%' . $q . '%');
                        $Topics = $Topics->where(function ($query) use ($q, $stids) {
                            $query->where('title_' . Helper::currentLanguage()->code, 'like', '%' . $q . '%')
                                ->orwhere('seo_title_' . Helper::currentLanguage()->code, 'like', '%' . $q . '%')
                                ->orwhere('details_' . Helper::currentLanguage()->code, 'like', '%' . $q . '%')
                                ->orwherein('id', $stids);
                    ...
                    $Topics_count = $Topics->wherein('id', $tids)->count();
                        ...
                        foreach ($Topic->webmasterSection->customFields as $customField) {
                            ...
                            if (($cf_saved_val != "" || count($cf_saved_val_array) > 0) && ($customField->lang_code == "all" || $customField->lang_code == @Helper::currentLanguage()->code)) {
                                if ($customField->type == 12) {
                                    ...
                                    $fields_details .= "<div><strong>" . $cf_title . " : </strong>" . date('Y-m-d H:i:s', strtotime($cf_saved_val)) . "</div>";
                                } elseif ($customField->type == 4) {
                                    $fields_details .= "<div><strong>" . $cf_title . " : </strong>" . date('Y-m-d', strtotime($cf_saved_val)) . "</div>";
                                } else {
                                    if ($tpc_title == "") {
